'use client'

import { useCart } from '@/lib/cart/CartContext'

export default function CartButton() {
  const { itemCount, openDrawer } = useCart()

  return (
    <button
      onClick={openDrawer}
      className="relative text-[11px] sm:text-xs uppercase tracking-widest text-ink hover:text-clay inline-flex items-center gap-1.5 font-medium transition-colors"
      aria-label={`Cart, ${itemCount} item${itemCount === 1 ? '' : 's'}`}
    >
      <span className="relative">
        <svg className="w-5 h-5" fill="none" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" viewBox="0 0 24 24" stroke="currentColor">
          <path d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z" />
        </svg>
        {itemCount > 0 && (
          <span className="absolute -top-2 -right-2 bg-clay text-paper rounded-full min-w-[16px] h-4 px-1 flex items-center justify-center text-[9px] font-semibold leading-none">
            {itemCount}
          </span>
        )}
      </span>
      <span className="hidden sm:inline">Cart</span>
    </button>
  )
}
